...
colnames(country_n) <- c("Country", "Number of Companies")
...
Well, my suspicion was partially correct! We see that beans are grown and sourced in from the Northern part of South America. It also seems as if Asia, Australia, Central Africa, and the Southern part of North America may be a hot spot as well. In fact, hot spot might be the best phrase to describe bean growth. It appears that all cocoa beans are sourced in from warm areas, around the equator. Displayed below are the top 10 bean-growing countries.

```{r, echo = F}
kable(head(country_n,10))
```


The plot leaves me with two thoughts. First, I am suprised to see the U.S. colored red. I've never heard of cocoa bean growth here. A quick google search tells me that beans grow in Hawaii, California and in Puerto Rico. Makes sense, if we stick with the hot theme. 
...
Relatively short analysis here. I just found out about #TidyTuesday yesterday, so I didn't have much time to dive further. If I did, I would look into which countries are producing beans that have the highest rating. This data set also contained information on the ingredients contained in each candy bar. I might revisit this in the future and try to do some cool analysis with that. I've never created a word map before, so that might be my goal! 

Follow my code at https://github.com/oliverjohnw/tidy-tuesdays/blob/main/cocoa_beans.R. 
